...
2. Layer 3 GeoJSONの地図表示（デジタイズ済みデータ）
3. Valhallaでの bicycle ルーティング（道央圏）
4. 基本ナビゲーション（音声なし・地図表示のみ）

### Phase 2
5. Webインターフェイスでのサイクリングロード登録・編集
6. Layer 1（OSM Overpass）と Layer 2（北海道大規模自転車道）の重ね表示
7. 走行情報リアルタイム表示（速度・方向・経過時間）

### Phase 3
8. 勾配グラフ（OpenTopoData連携）
9. 消費カロリー算出
10. GPXエクスポート
11. 駐車場表示（Overpass）
